...
2026-09-02.** The Mac Mah manual prints `Zoom 000-255` and nothing else, the
first definition guessed `SmallToBig`, and the show sent 255 to every look -
which on this unit is the 6-degree end, so both washes ran as pencils. Mac Mah
is an Algam brand, and Algam's own **MW19x15Z** is the same head - 325 x 188 x
395 mm, 8,5 kg, 300 W, 15 or 23 channels in our exact order - with a manual
that says what Mac Mah's leaves out (manualslib 3158887, pages 16-18): *Zoom
000-255 from wide to narrow*, strobe 0-9 off and 10-255 slow to fast, the
Macro channel's table (0-29 no function, then colours, colour changes and four
auto effects) and reset at 250-255. The definition carries all of it now and
`zoom_wide_pairs` writes 0. Two other programs' libraries were read on the
way: ChamSys MagicQ's personality agrees on the zoom (*Wide to Narrow*) and
disagrees on strobe bands and reset, and Lightkey's Algam profile agrees on the
macro table and has the zoom unflagged, which in its encoding reads as 0 =
narrow. The manufacturer's table wins on every count; the two libraries are
what led to it.
...
| Generic BEAM 230W 7R | **Verified on the hardware by the owner**, who states he tested the channel order when he wrote the definition (2026-08-25). The hand-built show corroborates the two channels that matter most: `Luz ON Cabezas` sends 255 to channel 6 and 255 to channel 7, `Luz OFF Cabezas` sends 0 to both - so channel 6 is the shutter, shut at 0, and channel 7 is the dimmer. No manual matches it (one found for a Rambo 230 puts Color on ch1 and Pan on ch10), so the hardware is the only reference there will be. A second search on 2026-09-01 over a dozen Sharpy-clone manuals found none whose 16-channel mode matches ours past channel 7: the closest is the official BeamZ Panther 7R manual, identical on channels 1-7 (`Pan, Tilt, Pan fine, Tilt fine, speed, strobe, dimmer`) and permuted from 8; the rest of the family (Ridgeyard, ETC/Hong Yi/Halo/YF) carries our exact *function set* - separate colour-effect and gobo-shake channels - in yet another order, with one shared value table for strobe, colour, gobo and prism. Lightkey's *All Stage Pro 230W 7R Beam Move Head* profile (2026-09-02) is the first chart found with our exact 16-channel order, and it corroborates the hardware on the strobe (0 closed, 1-50 open, 51-240 slow to fast, 241-255 open), the 8-value colour steps, the 128/192 rotation split on both wheels and the lamp at 100-105/200-205; it differs on the gobo step (10 values against the 7 measured) and the prism threshold (32 against 128), where what the owner measured stays. Then Daslight's library (decrypted with `ssl2-tools`, 2026-09-02) turned out to hold **six** profiles in our exact 16-channel order, and one of them - `_varied/Beam 230 SC-295-16CH` - matches the owner's definition **range for range**: strobe 0-2 closed, 3-50 open, 51-240 slow to fast, 241-255 open; colour steps of 8 with rotation at 128 and 192; gobo steps of 7 from 7-13; shake 1-128; prism in at 128; lamp off 100-105, lamp on 200-205, reset 206-255. So the hardware measurement is now also somebody else's chart. The one thing the chart says differently is **what the fourteen colours are**: SC-295 (and the Ridgeyard photo, and the whole ETC/Hong Yi family) names them white, red, orange, aquamarine, dark green, light green, lavender, pink, yellow, magenta, cyan, CTO 2, CTO 1, CTB, dark blue - the Clay Paky Sharpy wheel the clones copy - where our definition says red, orange, yellow, green, blue, pink, ice, light pink, light white, light blue, calid white, UV, grey, light yellow. `color_wheel_match` picks a beam's colour **by those names**, so if the chart is right, `Azul` on the beams is light green and `Amarillo` is aquamarine. Confirm on site (TODO). That shared table is the best guess for our unlabelled ranges (strobe `0-3 closed, 4-103 slow-fast, 104-107 open, 108-207 pulse, 213-251 random, 252-255 open`; prism `0-127 out, 128-255 in`; prism rotation `0-127 index, 128-190 one way, 191-192 stop, 193-255 the other`; reset above 128) and is only a guess until the probe says so. Physical side: see [Where the beams came from](#where-the-beams-came-from) |
| Vortex PC-64 LED S | No manual found anywhere - searched again on 2026-09-01 (manufacturer, Spanish shops, manual sites, second-hand ads, all spellings): "Vortex" as a stage-lighting brand has no web presence, so the name is the owner's label on a rebadged can. Generic 5-channel PAR64s come in **three incompatible layouts** - `R, G, B, Dimmer, Strobe` (what the definition says), `R, G, B, Dimmer, Strobe` plus macros in an 8-channel mode, and Stairville's `Mode, R, G, B, Speed` with no dimmer or strobe at all, whose 290 x 260 mm body is uncomfortably close to this one's 292 x 266 - so paper cannot settle it. The hand-built show strobed them on channel 5 and the owner saw them flicker, which fits the first layout and not the last. Only the channel probe on one can settles it; Daslight holds 273 five-channel `R, G, B, dimmer, strobe` PARs and not one named Vortex |
...
| Mac Mah MAC WASH 1915Z | Manual in `Manual/` (official, six languages): channel table and menu, no ranges. **Verified against the Algam MW19x15Z manual on 2026-09-02** - Mac Mah's parent brand, the same head to the millimetre and the gram - for zoom direction, strobe, macro table and reset; see the MAC WASH paragraph at the top of this page. Found by fingerprinting Lightkey's 7,704 profiles by channel order, where the Algam profile was the one exact 23-channel match. The manual itself is on manualslib (3158887), not downloadable; Algam's site only serves the 2025 `MW19x15ZX` ring-control variant, a different firmware. Daslight's library carries `Mac mah/MAC WASH 1915Z.ssl2`, the exact unit, coarse (strobe 0 off / 1-255, Function Mode 0 no function / 1-255, reset 255, zoom unsigned): it confirms the 23-channel order and that 0 is DMX control, nothing more |
...
reads as 0 = narrow in 45 of the 54 fixtures QLC+ also knows - a default, not a
measurement, which is how it got the MW19x15Z backwards.
...
are sold under a dozen names each.
